...
//                if ( in_array( $post_type, $meta_box->meta_box['object_types'] ) ) {  # TODO: Fix listing slider bug
                    ...
                    $description = apply_filters( 'inventor_metabox_description', $description, $metabox_key, $post_type );

                    // TODO: add setting to hide submission step if not allowed by user package
//                    if ( apply_filters( 'inventor_metabox_allowed', true, $metabox_key, get_current_user_id() ) ) {
...
//                    }
        ...
        if ( $valid_type && ! empty( $_GET['action'] ) && $_GET['action'] == 'save' ) {
            ...
            if ( ! empty( $_GET['id'] ) && ! empty( $step ) && ! empty( $type ) ) {
                ...
                if ( ! empty( $submission_list_page ) ) {
                    ...
                    // if not page does not exist, redirect to post detail
    ...
    public static function process_submission_save( $post_type ) {
        ...
        $title = self::get_submission_field_value( INVENTOR_LISTING_PREFIX . $post_type . '_general', INVENTOR_LISTING_PREFIX  . 'title' );
        $description = self::get_submission_field_value( INVENTOR_LISTING_PREFIX . $post_type . '_general', INVENTOR_LISTING_PREFIX  . 'description' );
        ...
        if ( ! empty( $post_id ) ) {
            ...
            if ( ! empty( $submission_data ) ) {
                ...
                foreach ( $submission_data as $metabox_id => $value ) {
                    ...
                    $cmb->save_fields( $post_id, $cmb->object_type(), $submission_data[ $metabox_id ] );
                }
     ...
     * @return mixed
     */
    public static function get_payload_data( $payload = null ) {
        $payload = $payload == null ? $_POST : $payload;
    ...
    public static function get_submission_data() {
        ...
        if ( 'COOKIE' == apply_filters( 'inventor_visitor_data_destination', INVENTOR_DEFAULT_VISITOR_DATA_STORAGE ) ) {
            ...
            $last_step_payload = self::get_payload_data();

            if ( is_array( $last_step_payload ) && count( $last_step_payload ) > 0 ) {
                $submission_data[ $current_metabox_id ] = $last_step_payload;
            }
        }
