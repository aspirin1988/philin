    ...
    public static function submission( $atts = array() ) {
        ...
        if ( empty( $object_id ) && ! Inventor_Submission_Logic::is_allowed_to_add_submission( get_current_user_id() ) ) {
            echo Inventor_Template_Loader::load( 'misc/not-allowed' , array(
                'message' => __( 'Check your package.', 'inventor-submission' )  // TODO: move to inventor-packages or use filter
            ));
        ...
        return cmb2_get_metabox_form( $meta_box, $object_id, array(
            'form_format' => '<form action="//' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] . $action . '" class="cmb-form submission-form" method="post" id="%1$s" enctype="multipart/form-data" encoding="multipart/form-data"> ' . $title . '<input type="hidden" name="object_id" value="%2$s">%3$s'. $submit_button .'</form>',
