...
class Inventor_Filter {
	...
	/**
	 * Returns price range from given params array
	 *
	 * @access public
	 * @param array $params
	 * @return array
	 */
	public static function get_price_range( $params ) {
		$price_from = null;
		$price_to = null;
		...
		if ( ! empty( $params ) && is_array( $params ) ) {
			...
			$price_from = count( $price_range ) > 1 ? $price_range[0] : ( empty( $params['price-from'] ) ? null : $params['price-from'] );
			$price_to = count( $price_range ) == 1 ? $price_range[0] : ( count( $price_range ) > 1 ? $price_range[1] : ( empty( $params['price-to'] ) ? null : $params['price-to'] ) );
		}

		return empty( $price_from ) && empty( $price_to ) ? null : array(
			'from' => $price_from,
			'to' => $price_to
		);
	}
	...
	public static function filter_query( $query = null, $params = null ) {
		...
		if ( ! empty( $params['distance-latitude'] ) && ! empty( $params['distance-longitude'] ) && ! empty( $params['distance'] ) ) {
			$distance_ids = array();
			$rows = self::filter_by_distance( $params['distance-latitude'], $params['distance-longitude'], $params['distance'] );
		...
		// Price
		$price_range = Inventor_Filter::get_price_range( $params );
