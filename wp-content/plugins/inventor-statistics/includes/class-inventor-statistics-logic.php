...
class Inventor_Statistics_Logic {
    ...
    public static function init() {
        ...
        add_action( 'inventor_before_listing_detail', array( __CLASS__, 'save_listing_view' ) );
        ...
        if ( is_array( $_GET ) && count( $_GET ) > 0 ) {
            foreach ( $_GET as $key => $value ) {
                if ( in_array( $key, $filter_fields_keys ) && ! empty( $value ) ) {
                    ...
                    // handle price value (multivalue)
                    if ( $key == 'price' ) {
                        $price_range = Inventor_Filter::get_price_range( $_GET );
                        foreach( array( 'from', 'to' ) as $range_key ) {
                            if( ! empty( $price_range[ $range_key ] ) ) {
                                $wpdb->insert( $wpdb->prefix . 'query_stats', array(
                                    'key'       => 'price_' . $range_key,
                                    'value'     => $price_range[ $range_key ],
